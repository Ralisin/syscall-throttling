...
flowchart LR
    ...
    hook -->|"verifica syscall, programma ed EUID"| state
...
| `kernel/state.c` | Conserva configurazione, programmi, EUID e syscall registrate; decide se una chiamata deve essere controllata. |
...
3. `state.c` valida i valori, applica la nuova configurazione in modo atomico e
   incrementa la generazione quando cambia lo stato osservabile.
...
2. Il pre-handler consulta `state.c` usando numero della syscall,
   `current->comm` ed EUID. Non dorme e non alloca memoria.
